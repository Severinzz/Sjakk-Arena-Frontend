...
export default {
  ...
  setPlayerInactive: ({ commit }, payload) => {
    tournamentService.inactivatePlayer('/tournament/set-inactive', payload.player_id).then(res => {
      commit('setPlayerInactive', payload.player_id)
    }).catch(err => {
      throw err
    })
  }
